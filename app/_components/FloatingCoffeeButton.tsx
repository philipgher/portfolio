...
export default function FloatingCoffeeMenu() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col-reverse items-end space-y-3 text-white">
      ...
      >
        <span className={`text-2xl transition-transform duration-300 ${open ? "rotate-45" : ""}`}>
          ☕
        </span>
      ...
      {contacts.map((contact, i) => (
        <a
          ...
          className={`flex items-center justify-center w-12 h-12 rounded-full text-white shadow-lg transform transition-all duration-300 ${open ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"
            } ${contact.color}`}
          style={{ transitionDelay: `${(open ? 1 : -1) * i * 0.02}s` }}
        >
          {contact.icon}
        </a>
      ))}
